...
export class MockListDatabase {
  ...
  createEmptyList(user: User): ListWithLinks {
    const newList: ListWithLinks = {
      ...
      emoji: getRandomEmoji(), // Random emoji 
      emoji_3d: undefined,
      is_public: false,
      price_cents: null,
      user_id: user.id,
  ...
  addLinkToList(listId: string, linkData: { url: string; title?: string }): ListWithLinks | null {
    ...
    const newLink = {
      ...
      favicon_url: `https://www.google.com/s2/favicons?domain=${new URL(linkData.url).hostname}&sz=32`,
      image_url: null,
      position: list.links.length,
      list_id: listId,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    }
